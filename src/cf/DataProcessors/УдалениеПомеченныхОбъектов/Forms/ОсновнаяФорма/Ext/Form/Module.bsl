...
Процедура РежимУдаленияПриИзменении(Элемент)
	Если РежимУдаления = "Полный" Тогда
		...
		Элементы.КомандаУдалить.Важность  = ВажностьКнопки.Главная;
...
Процедура ВыполнитьДалее()
	...
	ИначеЕсли ТекущаяСтраница = Элементы.РезультатыУдаления Тогда
		Элементы.КомандаДалее.Видимость        = Ложь;
		Элементы.КомандаУдалить.Видимость      = Ложь;
		Элементы.Закрыть.Важность              = ВажностьКнопки.Главная;
...
&НаСервере
Функция НайтиИлиДобавитьВетвьДереваСКартинкой(СтрокиДерева, Значение, Представление, Картинка)
	// Попытка найти существующую ветвь в СтрокиДерева без вложенных
	Ветвь = СтрокиДерева.Найти(Значение, "Значение", Ложь);
	Если Ветвь = Неопределено Тогда
		// Такой ветки нет, создадим новую
...
Процедура УдалениеВыбранныхНаСервере()
	...
	Для каждого Найденный Из Найденные Цикл
		...
		//ветвь метаданного
		СтрокаОбъектаМетаданных = НайтиИлиДобавитьВетвьДереваСКартинкой(Дерево.Строки, ОбъектМетаданныхНеудаленногоЗначение, ОбъектМетаданныхНеудаленногоПредставление, БиблиотекаКартинок.ОформлениеКругПустой);
		//ветвь не удаленного объекта
		СтрокаСсылкиНаНеудаленныйОбъектБД = НайтиИлиДобавитьВетвьДереваСКартинкой(СтрокаОбъектаМетаданных.Строки, Неудаленный, Строка(Неудаленный), БиблиотекаКартинок.ОформлениеКругЗеленый);
		//ветвь ссылки на не удаленный объект
		НайтиИлиДобавитьВетвьДереваСКартинкой(СтрокаСсылкиНаНеудаленныйОбъектБД.Строки, Ссылающийся, Строка(Ссылающийся) + " - " + ОбъектМетаданныхСсылающегося, БиблиотекаКартинок.ОформлениеКругКрасный);
	КонецЦикла;
	...
	Иначе
		...
		Элементы.КомандаДалее.Видимость         = Истина;
		Элементы.КомандаУдалить.Видимость       = Ложь;
		Элементы.КомандаДалее.Важность          = ВажностьКнопки.Обычная;
	КонецЕсли;
КонецПроцедуры
