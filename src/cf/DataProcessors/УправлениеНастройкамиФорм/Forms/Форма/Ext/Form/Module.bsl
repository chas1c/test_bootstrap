...
Функция ПолучитьМассивВыделенныхНастроек()
	...
	ВыделенныеЭлементы = Элементы.ОтфильтрованныеФормы.ПолучитьСтрокиДляОбработки();
...
&НаСервере
Процедура ПрименитьФильтр(СтрокаПоиска)
	
	ОтфильтрованныеФормы.Очистить();
	
	Для каждого ЭлементФорма Из Формы Цикл
		Если СтрокаПоиска = "" ИЛИ Найти(ВРег(ЭлементФорма.Представление), ВРег(СтрокаПоиска)) <> 0 Тогда
...
Асинх Процедура ВыполнитьКопирование()
	Если Элементы.ОтфильтрованныеФормы.ПолучитьСтрокиДляОбработки().Количество() = 0 Тогда
