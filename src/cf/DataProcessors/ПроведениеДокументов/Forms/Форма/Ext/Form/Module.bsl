...
&НаКлиенте
Процедура Исключить()
	Для каждого ИдентификаторЭлемента Из Элементы.ВыбранныеДокументы.ПолучитьСтрокиДляОбработки() Цикл
...
&НаКлиенте
Процедура Добавить()
	Для каждого ИдентификаторЭлемента Из Элементы.СписокДокументов.ПолучитьСтрокиДляОбработки() Цикл
...
Процедура ИсключитьВсе(Команда)
	Объект.ВыбранныеДокументы.Очистить();
	ПроверитьСостояниеКнопок();
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВсе(Команда)
	Для каждого ЭлементСпискаДокументов Из Объект.СписокДокументов Цикл
		Если Объект.ВыбранныеДокументы.НайтиПоЗначению(ЭлементСпискаДокументов.Значение) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;	
		Объект.ВыбранныеДокументы.Добавить(ЭлементСпискаДокументов.Значение, ЭлементСпискаДокументов.Представление);
	КонецЦикла;	
	Объект.ВыбранныеДокументы.СортироватьПоПредставлению(); 
	ПроверитьСостояниеКнопок();
КонецПроцедуры          
...
&НаКлиенте
Процедура ПроверитьСостояниеКнопок()

	Элементы.ДобавитьВсе.Доступность = Объект.СписокДокументов.Количество() > 0;
	Элементы.ИсключитьВсе.Доступность = Объект.ВыбранныеДокументы.Количество() > 0;
	
	Если Элементы.ДобавитьВсе.Доступность Тогда
		Элементы.ДобавитьВсе.Доступность = Объект.СписокДокументов.Количество() <> Объект.ВыбранныеДокументы.Количество()	
	КонецЕсли;  
КонецПроцедуры
