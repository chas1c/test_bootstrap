...
#Если (МобильноеПриложениеКлиент Или МобильныйКлиент) Тогда
	Элементы.Картинка.РастягиватьПоВертикали = Ложь;
#КонецЕсли
КонецПроцедуры
...
Процедура ДобавитьХарактеристику(Команда)
	...
	ОткрытьФорму("ПланВидовХарактеристик.ВидыХарактеристик.ФормаВыбора",
		,,,,, Оповещение, РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры
...
Асинх Процедура ВыполнитьУдалениеХарактеристики()
	...
	Для Каждого ОписаниеХарактеристики Из ОписаниеХарактеристик Цикл
		Если СтрНачинаетсяС(ТекущийЭлемент.Имя, ОписаниеХарактеристики.ИмяРеквизита) Тогда
			ЗначениеХарактеристики = ОписаниеХарактеристики.ПолучитьИдентификатор();
			НаименованиеХарактеристики = Строка(ОписаниеХарактеристики.ВидХарактеристики);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ЗначениеХарактеристики <> Неопределено Тогда
		Ответ = Ждать ВопросАсинх(НСтр("ru = 'Удалить характеристику """ + НаименованиеХарактеристики + """?'", "ru"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
		Если Ответ = КодВозвратаДиалога.Да Тогда
			//Выполним удаление
			УдалитьХарактеристикуНаСервере(ЗначениеХарактеристики);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьХарактеристики()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ПредыдущаяХарактеристика <> ВыборкаДетальныеЗаписи.Ссылка Тогда
			//Добавление горизонтальной группы для характеристики
			Группа = Элементы.Добавить(ИмяХарактеристики + "Группа", Тип("ГруппаФормы"), Элементы.ГруппаХарактеристики);
			...
			Элемент = Элементы.Добавить(ИмяХарактеристики, Тип("ПолеФормы"), Группа); 	
			Элемент.Вид = ВидПоляФормы.ПолеВвода;
			Элемент.Заголовок = ВыборкаДетальныеЗаписи.Наименование;
			Элемент.ПутьКДанным = ИмяХарактеристики;
			
			//Добавление кнопку для удаления характеристики
			Кнопка = Элементы.Добавить(ИмяХарактеристики + "Кнопка", Тип("КнопкаФормы"), Группа);
			Кнопка.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
			Кнопка.ИмяКоманды = "УдалитьХарактеристику";
			Кнопка.Отображение = ОтображениеКнопки.Картинка;
			Кнопка.Картинка = БиблиотекаКартинок.ПометитьНаУдаление;
...
Процедура ДобавитьХарактеристикуНаСервере(ВидХарактеристики)
	...
	Если ВидХарактеристики.Множественная Тогда
		Реквизит = Новый РеквизитФормы(ИмяХарактеристики, Новый ОписаниеТипов("СписокЗначений"),,, Истина);
	Иначе
		Реквизит = Новый РеквизитФормы(ИмяХарактеристики, ВидХарактеристики.ТипЗначения,,, Истина);
	КонецЕсли;
	ДобавляемыеРеквизиты.Добавить(Реквизит);
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
...
Процедура УдалитьХарактеристикуНаСервере(Идентификатор)
	...
	Элементы.Удалить(Элементы.Найти(ИмяРеквизита + "Кнопка"));
	Элементы.Удалить(Элементы.Найти(ИмяРеквизита + "Группа"));
...
&НаКлиенте
Процедура ШтрихкодНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	ТабличныйДокумент = ПечатнаяФормаШтрихкода();
...
&НаСервере
Функция ПечатнаяФормаШтрихкода()
	...
	Если Объект.ЭтоГруппа ИЛИ ПустаяСтрока(Объект.Штрихкод) Тогда 
		...
		Сообщение.Сообщить();
	Иначе
		РеквизитФормыВЗначение("Объект").ПечатнаяФормаШтрихкода(ТабличныйДокумент);
		Сформирован = Истина;
	КонецЕсли;	
