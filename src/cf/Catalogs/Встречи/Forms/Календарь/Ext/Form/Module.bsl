...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЦветТекущегоДня = ЦветаПалитры.ВторойБрендовый;
	ЦветНеТекущегоМесяца = ЦветаПалитры.Серый;
...
Процедура Назад(Команда)
	...
	ВыполнитьИзменениеПериодаОтображения(-1);
...
&НаКлиенте
Процедура ВыполнитьИзменениеПериодаОтображения(Вперед)

	Сутки = 24 * 60 * 60;
	Если ВариантПериода = "День" Тогда
		ДатаОтображения = ДатаОтображения + Вперед * Сутки;
	ИначеЕсли ВариантПериода = "Неделя" Тогда
		ДатаОтображения = ДатаОтображения + Вперед * Сутки * 7;
	ИначеЕсли ВариантПериода = "Месяц" Тогда
		ДатаОтображения = ДобавитьМесяц(ДатаОтображения, Вперед)
	КонецЕсли;
...
&НаКлиенте
Процедура ПерейтиКСегодняшнейДате(Команда)
...
Процедура ПланировщикНажатиеНаЭлементеИзмерения(Элемент, ЭлементИзмерения, ЗначенияИзмерений, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Пользователи.Форма.ФормаЭлемента", ПараметрыФормы,,,,,,РежимБлокировкиПриОткрытииОкнаФормы.БлокироватьВладельца);
...
Процедура УстановитьОтображениеПланировщика()
	...
	ИначеЕсли ВариантПериода = "Месяц" Тогда
		...
		Интервал.Цвет = ЦветНеТекущегоМесяца;
		...
		Пока ПользователиБазы.Следующий() Цикл
			Если Не ПользователиБазы.ПометкаУдаления = Истина Тогда
				...
				Если ТекущийПользовательБазы = ПользователиБазы.Код Тогда
					ЭлементИзмерения.Текст = Новый ФорматированнаяСтрока(ПользователиБазы.Наименование, Новый Шрифт(,, Истина), ЦветаПалитры.Серый);
				Иначе
	...
	Для Каждого Пользователь из МассивЗагружаемыхПользователей Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Планировщик.Элементы.Найти(Значение) = Неопределено Тогда
				...
				ЭлементПланировщика.ЦветТекста 	= ЦветаПалитры.Серый;
