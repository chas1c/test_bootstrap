...
#Если МобильныйАвтономныйСервер Тогда
	...
	Если ДанныеКонтакта.ВебАдреса.Количество() > 0 Тогда
		Объект.Сайт = ДанныеКонтакта.ВебАдреса[0].Значение;
	КонецЕсли;
...
#Если НЕ МобильныйКлиент Тогда
	...
	Элементы.ФормаГруппаГеопозиционирование.Видимость = Ложь;
	...
	Элементы.ФормаГруппаМультимедиа.Видимость = Ложь;
	...
	// Географические координаты уместны только на мобильном клиенте
...
Процедура АдресНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Страна", Объект.Страна);
	...
	ФормаВводАдреса = ПолучитьФорму("Справочник.Контрагенты.Форма.ФормаВводАдреса", ПараметрыФормы);
	ФормаВводАдреса.ОписаниеОповещенияОЗакрытии =
		Новый ОписаниеОповещения("ПриВводеАдреса", ЭтотОбъект);
	ФормаВводАдреса.Открыть();
...
&НаКлиенте
Процедура ПриВводеАдреса(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Объект.Дом = Результат.Дом;
		Объект.Индекс = Результат.Индекс;
...
Процедура СформироватьПредставлениеАдреса()
	...
	Адрес = "";
    Адрес = Адрес + ?(ЗначениеЗаполнено(Объект.Индекс), СокрЛП(Объект.Индекс) + ",","");
    Адрес = Адрес + ?(ЗначениеЗаполнено(Объект.Страна), " " + СокрЛП(Объект.Страна) + ",", "");
	Адрес = Адрес + ?(ЗначениеЗаполнено(Объект.Регион), " " + СокрЛП(Объект.Регион) + ",", "");
	Если СокрЛП(Объект.Регион) <> СокрЛП(Объект.Город) Тогда
		Адрес = Адрес + ?(ЗначениеЗаполнено(Объект.Город), " " + СокрЛП(Объект.Город) + ",", "");
	КонецЕсли;
	Адрес = Адрес + ?(ЗначениеЗаполнено(Объект.Улица), " " + СокрЛП(Объект.Улица) + ",", "");
    Адрес = Адрес + ?(ЗначениеЗаполнено(Объект.Дом), " " + СокрЛП(Объект.Дом) + ",", "");
