...
Функция ОписаниеВыделенныхФайлов()
	...
	Для Каждого Строка Из Элементы.Список.ПолучитьСтрокиДляОбработки() Цикл
...
Асинх Процедура ВыполнитьЗагрузитьФайлы()
	...
	Если РезультатПодключенияРасширенияРаботыСФайлами Тогда
		...
		Форма.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ЗагрузитьФайлыЗавершение", ЭтотОбъект);
		Форма.Открыть();
	Иначе
...
Процедура СкачатьАрхивом(Команда)
	...
	Если Файлы.Количество() > 0 Тогда  
		ВыполнитьСкачатьАрхивом(Файлы);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Асинх Процедура ВыполнитьСкачатьАрхивом(Файлы)
    ...
    Если Файлы.Количество() = 1 Тогда  
		ПараметрыАрхива.ИмяАрхива = Файлы[0].Имя + ".zip";
	КонецЕсли;
	
	РезультатПодключенияРасширенияРаботыСФайлами = Ждать ПодключитьРасширениеРаботыСФайламиАсинх();
	Если РезультатПодключенияРасширенияРаботыСФайлами Тогда
		Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
		МассивВыбранныхКаталогов = Ждать Диалог.ВыбратьАсинх();
		Если МассивВыбранныхКаталогов = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ИмяКаталога = МассивВыбранныхКаталогов[0];
		Попытка
			Ждать ПолучитьФайлыССервераАсинх(Файлы, ИмяКаталога, ПараметрыАрхива);
		Исключение
			Ждать ПредупреждениеАсинх(НСтр("ru='Ошибка получения архива'", "ru"));
		КонецПопытки;
	Иначе
		Попытка
			Ждать ПолучитьФайлыССервераАсинх(Файлы, Новый ПараметрыДиалогаПолученияФайлов(), ПараметрыАрхива);
		Исключение
			Ждать ПредупреждениеАсинх(НСтр("ru='Ошибка получения архива'", "ru"));
		КонецПопытки;
	КонецЕсли;
