...
Асинх Процедура ВыполнитьПрочитатьФайлИСохранитьНаДиск()
	...
	Если ПутьКФайлу <> "" И РезультатПодключенияРасширенияРаботыСФайлами Тогда
		...
		Ждать СоздатьКаталогАсинх(ПутьКФайлу);	
		...
		Иначе
			ТекстСообщения = НСтр("ru='Ошибка получения файла'", "ru");
		КонецЕсли;
	...
	Иначе
		Попытка
			Ждать ПолучитьФайлССервераАсинх(Адрес, Объект.ИмяФайла, Новый ПараметрыДиалогаПолученияФайлов());
		Исключение
			Ждать ПредупреждениеАсинх(НСтр("ru='Ошибка получения файла'", "ru"));
		КонецПопытки;
	КонецЕсли;	
КонецПроцедуры
...
Асинх Процедура ВыполнитьПодписать()
	...
	ФормаСпискаСертификатов.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("РезультатВыбораСертификатовДляПодписания", ЭтотОбъект, Контекст);
...
Асинх Процедура ВыполнитьПомещениеЗашифрованныйНаСервер()
	...
	ФормаСпискаСертификатов = ПолучитьФорму("Справочник.ХранимыеФайлы.Форма.СписокСертификатов", ПараметрыФормы);
	ФормаСпискаСертификатов.Установка(ПараметрыВыбораСертификатов);
	ФормаСпискаСертификатов.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("РезультатВыбораСертификатовДляПомещенияЗашифрованныйНаСервер", ЭтотОбъект, Контекст);
	ФормаСпискаСертификатов.Открыть();
КонецПроцедуры
...
Асинх Процедура ВыполнитьЗашифровать()
	...
	ПолучитьСписокСертификатов(ПараметрыФормы);
КонецПроцедуры
...
&НаКлиенте
Процедура ПолучитьСписокСертификатов(ПараметрыВыбора, Контекст = Неопределено)
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("МножественныйВыбор", Истина);
	ФормаСпискаСертификатов = ПолучитьФорму("Справочник.ХранимыеФайлы.Форма.СписокСертификатов", ПараметрыФормы);
	ФормаСпискаСертификатов.Установка(ПараметрыВыбора);
	ФормаСпискаСертификатов.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ЗашифроватьЗавершение", ЭтотОбъект, Контекст);
	ФормаСпискаСертификатов.Открыть();
КонецПроцедуры
...
Асинх Процедура ЗагрузитьФайлВременно(ПутьКЗагружаемомуФайлу)
	...
	ВременныйФайл = ПолучитьИмяВременногоФайлаНаСервере();
...
&НаСервере
Функция ПолучитьИмяВременногоФайлаНаСервере()
