...
Процедура УстановитьСтрокиСЧислом()
	...
	Элементы.КоличествоДней.ФорматРедактирования = "ЧФ='Ч " + СтрокаСЧислом(НСтр("ru = ';день;;дня;дней;дня'", "ru"), КоличествоДней, ВидЧисловогоЗначения.Количественное) + "'";
	Элементы.ИнтервалПроверкиИнтервалПроверкиЗаказов.ФорматРедактирования = "ЧФ='Ч " + СтрокаСЧислом(НСтр("ru = ';дня;;дней;дней;дней'", "ru"), Объект.ПериодПроверкиНеотработанныхЗаказов, ВидЧисловогоЗначения.Количественное) + "'"; 
